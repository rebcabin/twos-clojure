(ns twos-1-10-1.core-test
  (:require [clojure.data.priority-map       :refer      :all]
            ...
            [twos-1-10-1.core                :refer      :all]))

;;; see https://clojure.github.io/clojure/branch-master/clojure.spec-api.html,
;;; https://goo.gl/dyu59U

(def vt-gen (s/gen :twos-1-10-1.core/virtual-time))
...
(defspec minus-infinity-less-than-all-but-minus-infinity
  ...
  (prop/for-all
   [vt vt-gen]
   (if (not= vt Double/NEGATIVE_INFINITY)
     (< Double/NEGATIVE_INFINITY vt)
     true)))
;;; 2.
(defspec plus-infinity-not-less-than-any
  ...
  (prop/for-all
   [vt vt-gen]
   (not (< Double/POSITIVE_INFINITY vt))))
...
(deftest hand-constructed-message-expected-to-be-invalid
  ...
  (is (not (s/valid? :twos-1-10-1.core/message
                     (map->tw-message
                      {:sender       (new-uuid)
...
(deftest hand-constructed-message-conforms-to-spec
  (is (s/valid? :twos-1-10-1.core/message
                (map->tw-message
                 {:sender       (new-uuid)
                  :send-time    42
                  :receiver     (new-uuid)
...
(defspec message-pairs-identical-but-for-sign
  20
  (prop/for-all
   [[pos neg] (s/gen :twos-1-10-1.core/message-pair)]
   (and (=  1 (:sign pos))
        (= -1 (:sign neg))
        (= (:body         pos) (:body         neg))
...
(defspec all-messages-in-an-output-queue-are-negative
  20
  (prop/for-all
   [msg-vt-pairs (s/gen :twos-1-10-1.core/output-queue)]
   (every? (fn [[msg vt]] (= -1 (:sign msg)))
           msg-vt-pairs)))
...
(def iq-1
  (twos_1_10_1.core.input-queue.
   (into
    (priority-map)
    {(map->tw-message
      {:sender #uuid "d20f91ba-582f-4c64-a65f-f7d1d7892799",
       :send-time Double/NEGATIVE_INFINITY
     ...
     (map->tw-message
      {:sender #uuid "d7c9c04c-25ba-4f78-9d99-9685812eb769",
       ...
       :receive-time Double/POSITIVE_INFINITY
       :body nil,
       :sign 1,
...
(defspec inserting-a-new-input-message-into-iq-1-increases-its-count-to-fifty
  20
  (prop/for-all
   [m (s/gen :twos-1-10-1.core/input-message)]
   (= 50 (-> iq-1
             (insert-message m)
             :iq-priority-map
             count))))
...
(defspec all-message-ids-in-input-queue-are-unique
  20
  (prop/for-all
   [iq (s/gen :twos-1-10-1.core/input-queue)]
   (let [mid-set (reduce
